...
Fawns <- Adults %>% filter(Age.Class == "Neonate")
...
summary(survfit(Surv(days, Survival) ~ Sex, data = Fawns), times=43 )
...
summary(survfit(Surv(days, Survival) ~ Sex, data = Fawns), times=120 )
...
summary(survfit(Surv(days, Survival) ~ Sex, data = Fawns), times=180 )
...
####
Neonate_Sex <- survfit(Surv(days, Survival) ~ Sex,
                data = Fawns)
